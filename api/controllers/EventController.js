...
module.exports = {
  ...
  show: async (req, res) => {
    try {
      ...
      const data = await Post.findByIdHasJoinByEvent({ id, name });
  ...
  async allpay(req, res) {
    ...
    try {
      ...
      try {
        ...
        if (userNeedUpdate) {
          updateUserData = await updateUserData.save().catch(sequelize.UniqueConstraintError, (err) => {
      ...
      const findOrder = await Allpay.find({
        where: {
          PaymentType: '到店購買',
        },
        include: {
